...
#include <unistd.h>
#include <thread>
// #include "QueneroWalletListener.h"
#ifdef ANDROID
#include "../External/android/quenero/include/wallet2_api.h"
#else
#include "../External/ios/quenero/include/wallet2_api.h"
#endif

namespace Quenero = Wallet;
...
extern "C"
{
    ...
    struct QueneroWalletListener : Quenero::WalletListener
    {
        uint64_t m_height;
        bool m_need_to_refresh;
        bool m_new_transaction;

        QueneroWalletListener()
        {
            m_height = 0;
    ...
    struct TransactionInfoRow
    {
        ...
        TransactionInfoRow(Quenero::TransactionInfo *transaction)
        {
    ...
    struct PendingTransactionRaw
    {
        ...
        uint64_t fee;
        char *hash;
        Quenero::PendingTransaction *transaction;

        PendingTransactionRaw(Quenero::PendingTransaction *_transaction)
        {
    ...
    struct StakeRow
    {
        ...
        StakeRow(char *_masternode_key, uint64_t _amount)
        {
            masternode_key = _masternode_key;
            amount = _amount;
        }
    };
    ...
    struct StakeUnlockResult
    {
      bool success;
      char *msg;
      Quenero::PendingTransaction *pendingTransaction;

      StakeUnlockResult(bool _success, char *_msg,  Quenero::PendingTransaction *_pendingTransaction)
      {
        success = _success;
        msg = _msg;
    ...
    Quenero::Wallet *m_wallet;
    Quenero::TransactionHistory *m_transaction_history;
    QueneroWalletListener *m_listener;
    Quenero::Subaddress *m_subaddress;
    Quenero::SubaddressAccount *m_account;
    uint64_t m_last_known_wallet_height;
    uint64_t m_cached_syncing_blockchain_height = 0;
    std::mutex store_mutex;


    void change_current_wallet(Quenero::Wallet *wallet)
    {
        m_wallet = wallet;
        m_listener = nullptr;
    ...
    EXPORT
    Quenero::Wallet *get_current_wallet()
    {
        return m_wallet;
    }
    ...
    {
        Quenero::NetworkType _networkType = static_cast<Quenero::NetworkType>(networkType);
        Quenero::WalletManagerBase *walletManager = Quenero::WalletManagerFactory::getWalletManager();
        Quenero::Wallet *wallet = walletManager->createWallet(path, password, language, _networkType);

        auto stat = wallet->status();

        auto& [status, errorString] = stat;

        if (status != Quenero::Wallet::Status_Ok)
        {
    ...
    {
        Quenero::NetworkType _networkType = static_cast<Quenero::NetworkType>(networkType);
        Quenero::Wallet *wallet = Quenero::WalletManagerFactory::getWalletManager()->recoveryWallet(
            std::string(path),
            std::string(password),
            std::string(seed),
            _networkType,
    ...
    {
        Quenero::NetworkType _networkType = static_cast<Quenero::NetworkType>(networkType);
        Quenero::Wallet *wallet = Quenero::WalletManagerFactory::getWalletManager()->createWalletFromKeys(
            std::string(path),
            std::string(password),
            std::string(language),
            _networkType,
        ...
        if (status != Quenero::Wallet::Status_Ok || !errorString.empty())
        {
            error = strdup(errorString.c_str());
            return false;
        }
    ...
    {
        nice(19);
        Quenero::NetworkType networkType = static_cast<Quenero::NetworkType>(nettype);
        Quenero::Wallet *wallet = Quenero::WalletManagerFactory::getWalletManager()->openWallet(std::string(path), std::string(password), networkType);
        change_current_wallet(wallet);
    }
    ...
    {
        return Quenero::WalletManagerFactory::getWalletManager()->walletExists(std::string(path));
    }

    EXPORT
    void close_current_wallet()
    {
        Quenero::WalletManagerFactory::getWalletManager()->closeWallet(get_current_wallet());
    ...
    {
        nice(19);
        Quenero::Wallet *wallet = get_current_wallet();
    ...
    EXPORT
    bool stake_create(char *masternode_key, char *amount, Utf8Box &error, PendingTransactionRaw &pendingTransaction)
    {
        nice(19);

        Quenero::PendingTransaction *transaction;

        uint64_t _amount = Quenero::Wallet::amountFromString(std::string(amount));
        transaction = m_wallet->stakePending(std::string(masternode_key), _amount);

        int status = transaction->status().first;

        if (status == Quenero::PendingTransaction::Status::Status_Error || status == Quenero::PendingTransaction::Status::Status_Critical)
        {
    ...
    EXPORT
    bool can_request_stake_unlock(char *masternode_key)
    {
        std::unique_ptr<Quenero::StakeUnlockResult> stakeUnlockResult{m_wallet->canRequestStakeUnlock(masternode_key)};
        return stakeUnlockResult->success();
    }

    EXPORT
    bool submit_stake_unlock(char *masternode_key, Utf8Box &error, PendingTransactionRaw &pendingTransaction)
    {
        std::unique_ptr<Quenero::StakeUnlockResult> stakeUnlockResult{m_wallet->requestStakeUnlock(masternode_key)};
    ...
    void set_listener()
    {
        ...
        m_listener = new QueneroWalletListener();
    ...
    {
        std::vector<Quenero::SubaddressRow *> _subaddresses = m_subaddress->getAll();
        size_t size = _subaddresses.size();
        int64_t *subaddresses = (int64_t *)malloc(size * sizeof(int64_t));

        for (int i = 0; i < size; i++)
        {
            Quenero::SubaddressRow *row = _subaddresses[i];
    ...
    {
        std::vector<Quenero::SubaddressAccountRow *> _accounts = m_account->getAll();
        size_t size = _accounts.size();
        int64_t *accounts = (int64_t *)malloc(size * sizeof(int64_t));

        for (int i = 0; i < size; i++)
        {
            Quenero::SubaddressAccountRow *row = _accounts[i];
    ...
    {
        std::vector<Quenero::TransactionInfo *> transactions = m_transaction_history->getAll();
        size_t size = transactions.size();
        int64_t *transactionAddresses = (int64_t *)malloc(size * sizeof(int64_t));

        for (int i = 0; i < size; i++)
        {
            Quenero::TransactionInfo *row = transactions[i];
    ...
    {
        Quenero::Utils::onStartup();
        Quenero::WalletManagerFactory::setLogLevel(0);
    }
